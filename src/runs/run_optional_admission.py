...
import numpy as np
import pandas as pd
from assistants import MedAssistant, PatientAssistant, PatientContext, call_chat
...
from tool_execs import (
    close_case,
    generate_routine_optional_admission,  # other prompt
...
func_name_to_func = {
    ...
    "VitalSigns": get_vitalsign_results,
    ...
    "Plan": generate_routine_optional_admission,
...
def generate_patient_resource(row: pd.Series, practitioner_id: str) -> Patient:
    ...
    birth_year = 2025 - row["age"].values[0]
...
def prepare_patient(
    ...
    **kwargs,  # to catch overflowing arguments
    ...
    try:
        raw_gender = patient_data.patients.gender.values[0]
    except Exception:
        try:
            raw_gender = patient_data.patients.sex.values[0]
        except Exception:
            raw_gender = None
    if raw_gender is not None and pd.notna(raw_gender):
        raw_gender = str(raw_gender).strip()
    ...
    anamnesis_summary += (
        f"\n\nYour current pain level: {patient_data.triage['pain'].values[0]}\n\n"
    )
    ...
    # Setup collector
    hadm_id = patient_data.patients.hadm_id.values[0]
    ...
    if gender:
        demographic_lines.append(f"Sex: {gender}")
...
def patient_iterator(**kwargs):
    ...
    ds: MIMIC_Dataset_Admission_Experiments = kwargs.get("ds")
...
def run_simulations(
    ...
    Args:
        ...
        ds (MIMIC_Dataset_Admission_Experiments): The dataset containing patient information.
