...
if not EVALUATION_MODE:
    raise ValueError("Interactive mode is not included. Set EVALUATION_MODE=True.")
...
def prepare_patient(
    ...
    **kwargs,  # to catch overflowing arguments
    ...
    if age:
        patient_instructions = patient_instructions.rstrip() + f"\n\nCurrent age: {age}"
    ...
    def _change(sex):
        match sex:
            ...
            case _:
                raise ValueError(
                    "Other options currently not supported in this experiment."
                )
