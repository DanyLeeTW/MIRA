...
class MIMIC_Dataset(BaseModel):
    ...
    def save_dataset(self, name: str | None = None, overwrite: str = "ask"):
        ...
        valid_overwrite = {"ask", "yes", "no"}
        if overwrite not in valid_overwrite:
            raise ValueError(
                f"overwrite must be one of {sorted(valid_overwrite)}, got {overwrite!r}"
            )

        if path.exists():
            if overwrite == "no":
                print(f"Skipping dataset save for {name} (overwrite=no).")
                return
    ...
    def save_dataset_csv(self, name: str):
        ...
        for attr_name, attr_value in tqdm(self.__dict__.items()):
            if isinstance(attr_value, pd.DataFrame):
                # handle mixed types which break in the files
                # if one column has more than 1 type, convert it to string
                for col in attr_value.select_dtypes(include=["object"]).columns:
                    if attr_value[col].apply(type).nunique() > 1:
                        print(
    ...
    @classmethod
    def load_dataset(cls, name: str) -> MIMIC_Dataset:
