### in parts from:
### https://github.com/paulhager/MIMIC-Clinical-Decision-Making-Dataset/blob/main/tools/utils.py
...
import pandas as pd
from dataset.extracters import extract_rad_events
from dataset.utils import map_note_id_to_name
...
MODALITY_EXACT_DICT = {
    ...
    "Ultrasound": ["us", "dup", "BILAT LOWER EXT VEINS"],
...
def count_radiology_modality_and_organ_matches(row):
    text = row["note_name"]
    ...
    row["frequent_modality"] = frequent_modality
    row["frequent_modality_count"] = frequent_modality_count
    row["frequent_region"] = frequent_region
    row["frequent_region_count"] = frequent_region_count
    return row
...
def process_radiology(
    ...
    radiology["extracted_rad_events"] = radiology["text"].progress_apply(
        extract_rad_events
    )
    if hadm_ids is not None:
        assert set(radiology.hadm_id.to_list()).issubset(
            set(hadm_ids)
        ), "There are unvalid hadm_ids in the radiology dataset"

    # map the note_id to a parent_note_id
    note_id_to_parent_note_id = (
        radiology_detail[radiology_detail.field_name == "parent_note_id"]
        .set_index("note_id")["field_value"]
        .to_dict()
    )

    exam_name_map = (
        radiology_detail[radiology_detail.field_name == "exam_name"]
        .set_index("note_id")["field_value"]
        .to_dict()
    )

    # find the note_name for all radiology notes, either from the "note_id" or from the "parent_note_id" -> "note_id"
    radiology.loc[:, "note_name"] = radiology["note_id"].apply(
        lambda note_id: map_note_id_to_name(
            note_id, note_id_to_parent_note_id, exam_name_map
        )
    )

    # similar as to here: https://github.com/paulhager/MIMIC-Clinical-Decision-Making-Dataset/blob/main/dataset/dataset.py
    radiology = radiology.progress_apply(
        count_radiology_modality_and_organ_matches, axis=1
    )

    radiology.loc[radiology["frequent_modality_count"] == 0, "frequent_modality"] = None
    radiology.loc[radiology["frequent_region_count"] == 0, "frequent_region"] = None

    # Rename columns
    radiology = radiology.rename(
        columns={"frequent_modality": "modality", "frequent_region": "region"}
    )

    return radiology


def sanitize_radiology_entries(df, sanitize_list: List[str]):
    # Remove radiology entries that are invalid (no modality, no region, no rad events)
    # subj_id_removed_cnt = defaultdict(int)
    total_removed = 0

    def _check_invalid(row):
        nonlocal total_removed
        # check if one of the rows in the radiology list is invalid
        # subj_id = row["subject_id"]

        for disease_name in sanitize_list:
            row["extracted_rad_events"] = re.sub(
                re.compile(disease_name, re.IGNORECASE),
                "____",
                row["extracted_rad_events"],
            )

        is_invalid = (
            row["modality"] is None
            or row["region"] is None
            or row["extracted_rad_events"] == ""
        )
        if is_invalid:
            # subj_id_removed_cnt[subj_id] += 1
            total_removed += 1
        return is_invalid

    df["is_invalid"] = df.progress_apply(_check_invalid, axis=1)
    print(f"Total number of radiology reports removed: {total_removed}")
    return df
