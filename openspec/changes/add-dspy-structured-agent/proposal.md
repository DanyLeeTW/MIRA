...
在 repo 根目录新增一个独立的 `dspy/` 包，把 MIRA agent 的 tool-calling/prompting 逻辑用 DSPy 的 Signature/Module/ReAct 结构重写一遍，作为跟 `src/`（论文复现参照实现）平行、可独立编译优化的实现。范围限定在"开单 + 诊断"这一段（对应现在 `generate_routine()` + `MedAssistant.chat()`）；问诊环节（`conv.py` 的 doctor↔patient ping-pong）保持冻结，不参与这轮优化。
...
- 新建 `dspy/` 作为独立可安装包，依赖 `hospitalagent-src`（`src/` 现有包名，`uv pip install -e ./src`）+ `dspy-ai`。
...
- **把 metric reduction 函数放进 `src/evaluations/`**：逻辑上是 `objectives.py` 里 set-overlap dicts 的自然延伸，论文自身的 evaluation notebook 也能顺带受益。但违背"`src/` 保持不动"的原则；改为在 `dspy/metrics.py` 里 import `src/evaluations/objectives.py` 的 matching 逻辑，只加一层 reduce-to-scalar。

## Impact

- [ ] Breaking changes — 无，`src/` 及其现有行为不受影响，纯新增一个平行包
- [ ] Database migrations — 无
- [ ] API changes — 无（新增 `dspy/` 包，不改动任何现有对外接口）
...
- `src/runs/run.py`/`run_with_sex_bias.py`/`run_optional_admission.py` 85-90% 重复的清理，不在本次范围内（`dspy/runs/compile_and_run.py` 会用参数化设计规避同样的重复，但不回头改 `src/`）。
